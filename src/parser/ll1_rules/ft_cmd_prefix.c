...
/*   Updated: 2022/01/25 15:20:05 by sde-alva         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_cmd_prefix(t_syntax *stx_vars, enum e_symbol tok_type)
{
	if (tok_type >= 0)
	{
		...
		ft_lstpush(&stx_vars->symbol_stack, ft_stk_node(NTS_PREFIX1));
		if (tok_type == TS_ASSIGNMENT)
		{
			ft_lstpush(&stx_vars->symbol_stack, ft_stk_node(TS_ASSIGNMENT));
		}
		else
		{
