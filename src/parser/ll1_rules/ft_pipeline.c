...
/*   Updated: 2022/01/26 21:19:29 by sde-alva         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_pipeline1(t_syntax *stx_vars, enum e_symbol tok_type)
{
	...
	if (tok_type >= 0)
	{
		...
		stk_node_old->ast_node->children++;
		stk_node_old->ast_node->first_child->next_sibling = ft_stk_node(NTS_PIPELINE);
		stk_node = (t_stk_node *)ft_stk_node(tok_type);
