...
/*   Updated: 2022/01/25 15:22:56 by sde-alva         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_simple_cmd(t_syntax *stx_vars, enum e_symbol tok_type)
{
	if (tok_type >= 0)
	{
		...
		if (tok_type == TS_WORD)
		{
			ft_lstpush(&stx_vars->symbol_stack, ft_stk_node(NTS_SIMPLE_CMD2));
			ft_lstpush(&stx_vars->symbol_stack, ft_stk_node(TS_WORD));
		}
		else
		{
			ft_lstpush(&stx_vars->symbol_stack, ft_stk_node(NTS_SIMPLE_CMD1));
			ft_lstpush(&stx_vars->symbol_stack, ft_stk_node(NTS_PREFIX));
		}
	}
}
