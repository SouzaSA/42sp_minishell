...
/*   Updated: 2022/02/26 18:40:29 by sde-alva         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_cmd_iter(t_shell *shell, t_cmd_data *cmd_data);
static void	ft_cmd_send(t_shell *shell, t_cmd_data *cmd_data, t_ast *ast);
...
static int	ft_run_cmds(t_shell *shell, t_list **cmd_stk)
{
	...
	if (ft_lstsize(*cmd_stk) == 1)
	{
		...
		if (ast->cmd && ast->cmd->cmd && ft_isbuiltin((char *)(ast->cmd->cmd->content)))
		{
			...
			ft_assignments(shell, ast->cmd->assign);
			ft_redirections(ast->cmd->redir, &cmd_data.fd_in, &cmd_data.fd_out);
...
static int	ft_cmd_iter(t_shell *shell, t_cmd_data *cmd_data)
{
	...
	rtn = 0;
	while (rtn == 0 && ft_lstsize(*cmd_data->cmd_stk))
	{
		ast = (t_ast *)ft_lstpop(cmd_data->cmd_stk);
		if (ast->type == AST_CMD && shell)
		{
			ft_cmd_send(shell, cmd_data, ast);
			ast = NULL;
		}
...
static void	ft_cmd_send(t_shell *shell, t_cmd_data *cmd_data, t_ast *ast)
{
	...
	ast_nxt = (t_ast *)ft_lsttop(*cmd_data->cmd_stk);
	cmd_data->pipe_flag = 0;
	if (ast_nxt && ast_nxt->type == AST_PIPE)
		cmd_data->pipe_flag = 1;
	cmd_data->cmd_stk = cmd_data->cmd_stk;
